...
def main(
    name: str = typer.Option("mspacman", help='Experiment name'),
    rand_seed: int = typer.Option(123, help='Random rand_seed'),
    total_steps: int = typer.Option(5000000, '--T-max', help='Number of training steps'),
    ...
    memory_capacity: int = typer.Option(1000000, help='Experience replay memory capacity'),
    ...
    target_update: int = typer.Option(10000, help='Steps between target network updates'),
    reward_clip: int = typer.Option(0, help='Reward clipping (0 to disable)'), 
    learning_rate: float = typer.Option(0.000025, help='Learning rate'), 
    ...
    learn_start: int = typer.Option(50000, help='Steps before starting training'), 
    evaluate: bool = typer.Option(False, help='Evaluate only'),
    evaluation_interval: int = typer.Option(50000, help='Steps between evaluations'), 
    evaluation_episodes: int = typer.Option(3, help='Number of evaluation episodes'), 
    evaluation_size: int = typer.Option(500, help='Transitions for validating Q'), 
    render: bool = typer.Option(False, help='Display screen'), 
    checkpoint_interval: int = typer.Option(100000, help='How often to checkpoint (0=disable)'), 
    memory_path: str = typer.Option('replay_memory', help='Path to save/load memory'), 
    annealing_end: int = typer.Option(4000000, help='Step for beta annealing end (0=disable)'), 
    learn_shlr: bool = typer.Option(True, help='Enable learning rate scheduling'),
):
    options = locals()
